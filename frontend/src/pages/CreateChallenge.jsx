...
import { CheckCircle2, MapPin, PlusCircle, ShieldAlert, SlidersHorizontal, Upload } from 'lucide-react';
...
const CreateChallenge = () => {
  ...
  const submitReport = async (event) => {
    ...
    const finalTitle = title.trim() || description.trim().split(/[.!?]/)[0].slice(0, 120);
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
        <div className="mb-6 pb-4 border-b border-slate-100">
          ...
          <p className="text-sm text-slate-500 mt-1">Add a photo, exact location, and a short explanation to report an issue.</p>
        </div>
        ...
        <form onSubmit={submitReport} className="space-y-5">
          <div><label className="block text-xs font-bold text-slate-800 uppercase tracking-wider mb-1">Short title <span className="normal-case text-slate-400 font-normal">(optional—your first sentence is used if blank)</span></label><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Broken handpump near the middle school" className="w-full px-4 py-2.5 text-sm border border-slate-300 rounded-xl bg-slate-50/50" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-700">Category *<select value={category} onChange={(event) => setCategory(event.target.value)} className="mt-1 block w-full p-2.5 border border-slate-300 rounded-xl bg-slate-50/50">{CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-xs font-bold text-slate-700">Severity *<select value={severity} onChange={(event) => setSeverity(event.target.value)} className="mt-1 block w-full p-2.5 border border-slate-300 rounded-xl bg-slate-50/50">{SEVERITIES.map((item) => <option key={item}>{item}</option>)}</select></label></div>
          ...
          <button type="submit" disabled={loading} className="w-full py-3.5 px-6 bg-brand-600 hover:bg-brand-700 text-white font-black rounded-xl shadow-md disabled:opacity-50">{loading ? 'Submitting report…' : 'Submit report'}</button>
          <p className="text-center text-[11px] text-slate-500 flex justify-center items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> Review the information before submitting your report.</p>
        </form>
      </div>
    </div>
  );
